...
public class GeoHashGeometry {
    ...
    public List<String> encode(Geometry geometry,boolean inclParents){
        if (geometry instanceof Point){
            ...
            List<String>  geoHashs = new ArrayList<String>(level) ;
            for (int i=1;i<=geohash.length();i++){
                geoHashs.add(geohash.substring(0,i)) ;
            }
            return geoHashs ;
        }
    ...
    public List<String> encodeSubChild(String geohash,Geometry geometry,boolean incParents){
        List<String> geoHashs = new ArrayList<String>() ;
        matchGeometry(geohash,geometry,geoHashs,incParents);
        return geoHashs ;
    }
        ...
        for (int i=0;i<geohash.length();++i){
            ...
            for (int bit:BITS){
                if (isEven){
                    ...
                    if ((dec&bit)!=0){
                        latRange[0] = (latRange[0]+latRange[2])/2.0 ;
                    }else{
